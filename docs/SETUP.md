...
O caminho padrão de imagem é o ComfyUI: RealVisXL, Realistic Vision e FLUX.2 Klein são instalados pelos scripts da seção de geração de imagens. No menu rápido do chat, porém, você também pode escolher **Modelo direto**. Nesse modo o Avento chama a API OpenAI-compatível <code>/v1/images/generations</code> do modelo local selecionado; ele exige um modelo/servidor local que realmente suporte geração de imagem, não um modelo textual comum do Ollama. Provedores remotos continuam usando o ComfyUI local para mídia nesta versão. A origem escolhida aparece no seletor e no card do job, para não confundir uma geração no ComfyUI com uma geração direta.
...
O botão de clipe do chat envia PDF, Office, EPUB, ZIP, texto e código para `POST /api/documents/extract`. O backend lê texto simples diretamente e usa o MarkItDown local nos formatos binários, mantendo apenas uma cópia temporária durante a extração. A instalação padrão fica em `~/.avento/tools/mcp`; `AVENTO_LOCAL_MCP_TOOLS_DIR` e `AVENTO_MARKITDOWN_COMMAND` podem apontar para outro local. Como ambientes virtuais Python guardam caminhos absolutos, `scripts/setup-local-mcps.sh` valida os comandos e reconstrói automaticamente a instalação quando necessario. O limite HTTP é 50 MB por arquivo e o trecho entregue ao modelo pode ser ajustado com `AVENTO_DOCUMENT_ATTACHMENT_MAX_CONTEXT_CHARS` (padrão: 5.000 caracteres).
...
O backend compacta historico antigo, remove prompts de sistema duplicados do frontend e envia apenas as ferramentas relevantes para o pedido antes de chamar o modelo. Continuacoes curtas preservam o ultimo pedido explícito em `[Conversation Continuity]`; quando a resposta anterior terminou no meio de uma estrutura Markdown, essa continuidade recebe o trecho final e a instrução de retomar sem repetir a introdução. A mesma guarda faz uma única tentativa automática, sem ferramentas, antes de persistir resposta com delimitador aberto. `avento.agent.num-ctx` define o teto local da janela compartilhada entre entrada e saida; para modelos locais, o Avento usa o menor valor entre esse teto e a janela declarada pelo modelo. O padrão local é 16.384 tokens. Antes da chamada, o Avento reserva integralmente `avento.agent.num-predict` (4.096 por padrão) para a geração, incluindo o raciocínio interno de Thinking, e reduz o histórico antigo quando a estimativa do prompt ameaça essa margem. Máquinas com memória de sobra podem elevar `AVENTO_AGENT_NUM_CTX`; em uma máquina de 16 GB, não use a janela teórica de 128k/262k declarada por modelos locais sem medir a memória. Também podem ser ajustados `temperature`, `top-p`, `top-k`, `repeat-penalty` e os três limites de compactação. O RAG aplica `avento.rag.similarity-threshold` antes de incluir trechos no contexto, reduzindo resultados semanticamente fracos.
...
avento:
  agent:
    # Teto local; o Avento respeita também a janela declarada pelo modelo e reserva a geração.
...
Se voce souber apenas o nome de um projeto, pode pedir para o Avento localizá-lo no Mac sem informar
o caminho. A ferramenta nativa `find_local_project` procura somente nomes de pastas dentro do
diretório do usuário atual, não varre o disco inteiro, não lê arquivos e ignora `Library`,
`node_modules`, `.git`, `build`, `dist`, `target` e caches similares. Ela devolve no máximo 20
resultados e não transforma nenhum deles em workspace: você escolhe a pasta antes de abrir,
indexar ou editar.
...
As ferramentas locais essenciais do agente ficam disponiveis pelo backend mesmo quando um servidor MCP externo nao conecta: `directory_tree`, `read_file`, `write_file`, `delete_file`, `create_directory`, `search_files`, `find_local_project`, `create_vite_project`, `open_app`, `close_app`, `open_url`, `open_path`, `reveal_in_finder`, `run_shortcut`, `capture_screen`, `terminal_run`, `terminal_start`, `terminal_list`, `terminal_logs` e `terminal_stop`. Todas as ferramentas de arquivo exigem que o caminho esteja dentro de um workspace autorizado, com uma exceção deliberada: `find_local_project` só lê nomes de diretórios sob a pasta do usuário atual para descobrir um projeto cujo caminho ainda é desconhecido; ela não lê conteúdo, não aceita uma raiz informada pelo modelo e não autoriza o resultado. `write_file` cria diretorios pais quando necessario e registra backup antes de sobrescrever. `delete_file` remove apenas arquivo regular autorizado e tambem gera backup antes de apagar. `open_path` e `reveal_in_finder` tambem ficam presos a workspaces autorizados. `open_app`, `close_app`, `open_url`, `run_shortcut` e `capture_screen` usam automacao local do macOS sem shell livre; URLs aceitam apenas `http` e `https`, e screenshots sao salvos em `~/Pictures/Avento Screenshots`. `create_vite_project` e um scaffold controlado para templates Vite conhecidos, como `react-ts`, sem liberar shell arbitrario. `terminal_run` tambem nao e shell livre: ele aceita apenas comandos curtos permitidos como `npm create vite@latest ...`, `npm install`, `npm run build/test`, `mvn test`, `git status` e alguns comandos seguros de Docker Compose. Processos longos como `npm run dev` devem usar `terminal_start` e ser acompanhados por `terminal_list`, `terminal_logs` e `terminal_stop`. Quando o Docker MCP Gateway conectado anunciar pelo `tools/list` o mesmo `originalName` de uma ferramenta nativa, o agente recebe uma unica capacidade com o nome canônico e a chamada segue para o Docker; se nao houver anuncio exato, a nativa continua ativa. Uma falha no Docker e devolvida ao chat, sem repetir a operacao no host.
